...
save.path <- file.path("/Users/charliewilson/Dropbox/Reversible Cognition/`Data for Iris/Domenech_Analysis"); #Path to save your data frame. Try not to put it in with the raw data
...
```{r}
 Sess.content <- Data %>%
        ...
        summarize(max.sn = max(trial), no65 = count(event=65)) 
...
colnames(Sess.dat) <- c("session","xx","152","yy","153"); #Rename columns
...
Data <- Data %>%
  ...
  dplyr::mutate(GTrap = sum(event == 152), BTrap = sum(event == 153)) %>%
  dplyr::ungroup() 
